...
function parse_name($name, $type=0) {
    if ($type) {
        return ucfirst(preg_replace("/_([a-zA-Z])/e", "strtoupper('\\1')", $name));  // '\\1'为后向引用
    } else {
        $name = preg_replace("/[A-Z]/", "_\\0", $name);  // 将字符串中的大写字母替换为小写字母或下划线
        return strtolower(trim($name, "_"));  // C规定变量名由字母和数字组成，下划线被看作字母，第一个字符必须为字母（除下划线）
    }
}
...
function compile($filename, $runtime=false) {
    ...
    $content = substr(trim($content), 5);   // 去除'<?php'
    if ('?>' == substr($content, -2))
