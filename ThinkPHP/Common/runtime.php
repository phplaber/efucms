...
// 检查缓存目录(Runtime)是否可写并自动创建目录下的四个目录
...
function build_runtime() {
    ...
    if(version_compare(PHP_VERSION,'5.2.0','<') )
        $list[]	= THINK_PATH.'/Common/compat.php'; // 向数组追加元素
    // 加载核心编译文件列表
    ...
    if(!defined('NO_CACHE_RUNTIME')) {
        $compile = defined('RUNTIME_ALLINONE'); // return 'true' or 'false'
...
if (!function_exists('build_action'))
{
    function build_action()
    {
		...
		// 'BUILD_MODE'可设为'BuildModeIndex'或'DefaultIndex'，分别创建不同的操作
...
/*要点：
1，加载基本文件（系统定义文件：defines.php和项目目录与路径定义文件：paths.php）和核心文件（core.php，其中包含所有的核心文件）
2，核心文件有三种不同的形式：自定义，模式扩展和默认形式
3，自动创建项目目录结构
4，生成核心缓存文件（将加载的所有文件整合到一起，并去除注释和空白，提高程序性能）
5，循环加载文件和循环生成目录
*/
?>